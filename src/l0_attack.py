...
class CarliniL0:
    ...
    def gradient_descent(self, sess, model):
        ...
        outgrad = tf.compat.v1.gradients(loss, [modifier])[0]
        ...
        def doit(oimgs, labs, starts, valid, CONST):
            ...
            while CONST < self.LARGEST_CONST:
                ...
                for step in range(self.MAX_ITERATIONS):
                    ...
                    #scores = tf.math.log(scores/(1-scores)).eval()
    ...
    def attack_single(self, img, target):
        ...
        #valid = np.ones(self.input_data_shape)
        valid = np.ones((1, 28, 28, 1))

        # the previous image
        #prev = np.copy(img).reshape(self.input_data_shape)
        ...
        while True:
            ...
            #equal_count = self.input_data_shape[-1]-np.sum(np.all(np.abs(img-nimg[0])<.0001))
            equal_count = (1, 28, 28, 1)[-1] - np.sum(np.all(np.abs(img - nimg[0]) < .0001))
            ...
            if self.independent_channels:
                ...
                #valid = valid.reshape((self.input_data_shape[-1], 1))
                valid = valid.reshape((28**2, 1))
                totalchange = abs(np.sum(nimg[0]-img))*np.sum(np.abs(gradientnorm[0]))
            ...
            #valid = np.reshape(valid, self.input_data_shape)
            valid = np.reshape(valid, (1, 28, 28, 1))
